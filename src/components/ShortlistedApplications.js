...
const ShortlistedApplication = () => {
  ...
  const rejectedApplications = AllApplications.filter(application => application.status === 'selected');

  // Pagination
  const indexOfLastApplication = currentPage * applicationsPerPage;
  const indexOfFirstApplication = indexOfLastApplication - applicationsPerPage;
  const currentApplications = rejectedApplications.slice(indexOfFirstApplication, indexOfLastApplication);
    ...
    <div>
      <div className='m-4 rounded-lg w-full lg:w-10/12 mx-auto p-4'>
        ...
        <div className="fixed bottom-6 right-6">
          <div className="bg-gradient-to-r from-purple-700 to-[#2E6C9D] p-4 rounded-full shadow-lg flex items-center space-x-2 animate-bounce">
            <span className="text-white text-lg font-bold">{rejectedApplications.length}</span>
            <span className="text-white">Succesfull Applications</span>
          </div>
        </div>
              ...
              <table className="w-full table-auto">
                <thead>
                  <tr className='border-b border-slate-500'>
                    ...
                    <th className='px-6 py-3'>Applicants Profile</th>
                  </tr>
                </thead>
                <tbody>
      ...
      <div className="flex justify-center">
        <ul className="flex items-center mt-4">
          {Array(Math.ceil(rejectedApplications.length / applicationsPerPage)).fill().map((_, index) => (
...
export default ShortlistedApplication
